...
class BaselinePlanningWorkflow:
    ...
    def run(self, case_config_path: str | Path) -> dict[str, Any]:
        ...
        metrics_path = self.repo_root / config.get("metrics", {}).get("path", "")
        metrics_config, metrics_warnings = self._load_metrics(metrics_path)
    ...
    def _load_metrics(self, path: Path) -> tuple[dict[str, Any], list[str]]:
        import yaml
